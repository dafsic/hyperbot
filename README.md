...
| `bot` | 事件循环编排：设杠杆 → 启动对账 → 播单 → 前沿轮询成交（每 10s）→ 风控 → 优雅退出 |
...
成交检测使用**前沿轮询（REST 纯轮询，无 websocket）**：

- **立即成交**：下单时交易所返回 `resting=false`，Bot 即时记录并挂出对手单。
- **挂单成交**：每 **10 秒** `reconcile` 对当前最前沿的挂单调用 `order_status`——卖单按价格升序取最低档，买单按价格降序取最高档，每侧最多 **1 次** API 调用。发现 `Filled` 后继续扫描下一单；遇到 `Open` 或 `PartialFill`（部分成交）则停止（前沿不变量：更靠后的档位一定未成交）。

这一设计将每轮 API 调用总量与网格档位数解耦，固定为 ≤2 次，避免限流风险。
